...
## 📸 Камера: як передавати знімки на ПК
1. **Запустіть сервіс на Orange Pi.** Він тепер видає кадри за `http://<opi-ip>:8000/camera/snapshot`. За замовчуванням бере `/dev/video0`, змінити можна змінною середовища `CAMERA_DEVICE` (ширину/висоту — `CAMERA_WIDTH`/`CAMERA_HEIGHT`).

2. **Перевірте, що кадр віддається.** На ПК виконайте:
   ```bash
   curl http://<opi-ip>:8000/camera/snapshot --output snapshot.jpg
   ```
   Якщо файл з'явився — камера доступна та стрім працює.

3. **Підключіть ПК-сервіс до цієї камери.** Встановіть IP Orange Pi у `.env` або під час запуску: `OPI_HOST=<opi-ip>`. Тоді `pc-llm-service` автоматично використає HTTP-знімок з Orange Pi замість локального `/dev/video0`.

4. **Альтернатива — USB-камера напряму на ПК.** Пропустіть Orange Pi і передайте пристрій у контейнер: `CAMERA_DEVICE=/dev/video0 docker compose -f docker-compose.pc.yml -f docker-compose.pc.camera.yml up -d pc-llm-service`.
