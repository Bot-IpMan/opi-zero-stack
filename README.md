...
> ℹ️ **План модернізації** вже доступний у файлі [ARCHITECTURE_uk.md](ARCHITECTURE_uk.md): нова гібридна архітектура без Orange Pi PC, LLM координатор на ПК, оновлений Orange Pi Zero та Arduino firmware.
...
- 🚀 **GPU**: 400+ CUDA cores → 100x швидше ніж CPU
- ⏱️ **Час**: 2-4 години замість кількох днів
- 💾 **Ресурси**: ПК має 16GB RAM, Orange Pi Zero має 512MB
- 🎯 **Один раз**: Навчимо один раз, потім розгортаємо везде

### Етап 1: Підготовка ПК

```bash
cd training

# Переконатись, що Dockerfile та requirements.txt існують
ls -la Dockerfile requirements.txt

# Збудувати Docker образ
docker compose -f ../docker-compose.train.yml build training
# ⏱️ Час: 5-10 хвилин
# Завантажує: PyTorch, TensorFlow, Stable-Baselines3, PyBullet
```

### Етап 2: Запуск навчання

```bash
# Запустити PPO навчання
make train
...
1. **Ініціалізація** (30 сек):
   ```
   ✅ PyBullet connection
   ✅ Gymnasium environment loaded
   ✅ PPO agent created (128x128 network)
   ```

2. **Навчання** (2-4 години):
   ```
   Iteration 1/500000 | Reward: -8.3 | Loss: 0.45
   Iteration 1000/500000 | Reward: -2.1 | Loss: 0.12
   Iteration 100000/500000 | Reward: 45.2 | Loss: 0.01
   Iteration 500000/500000 | Reward: 98.5 | Loss: 0.002 ✅
   ```

3. **Збереження** (5 сек):
   ```
   ✅ Model saved: training/models/ppo_model.zip
   ✅ Normalization saved: training/models/vec_normalize.pkl
   ```

### Етап 3: Моніторинг навчання (опціонально)
...
Графіки:
- 📈 **Episode Reward**: Мають зростати 0 → 100
- 📉 **Policy Loss**: Мають спадати (менше = краще)
- 📊 **Value Loss**: Мають бути стабільними

### Етап 4: Експорт моделей

**Коли навчання закінчилось:**

```bash
# Експортувати обидві моделі
make export

# Або вручну:
docker compose -f docker-compose.train.yml run --rm training \
  python export_models.py \
  --ppo-model training/models/ppo_model.zip \
  --ppo-output training/models/ppo_model.tflite \
  --yolo-output training/models/yolov8n.tflite
```

**Результати:**
```
✅ PPO TFLite: training/models/ppo_model.tflite (200 KB)
   - Inference time: 10-20ms
   - Input: [joint_angles(6), yolo_target(3)] = 9 values
   - Output: [new_angles(6)]

✅ YOLO TFLite: training/models/yolov8n.tflite (3 MB)
   - Inference time: 30-50ms
   - Input: [camera_frame (320x240)]
...
# ppo_model.tflite       200K  ← на Orange Pi Zero
# yolov8n.tflite         3M    ← на Orange Pi PC
```
