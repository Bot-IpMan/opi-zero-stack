...
Поле `serial.ack` міститиме рядки з підтвердженням від плати, що допоможе діагностувати
реакцію прошивки.
...
| `LOG_LEVEL`          | `INFO`                    | Рівень логування FastAPI застосунку. |
| `SERIAL_ACK_TIMEOUT` | `0.75`                    | Скільки секунд чекати на відповідь від Arduino після команди. |
| `SERIAL_ACK_POLL`    | `0.05`                    | Інтервал між перевірками буфера під час очікування ACK. |
