...
# ⚠️ Якщо змінну не вказати, `docker-compose.yml` підставить `/dev/null`,
# тож сервіс `yolo-detector` зможе стартувати навіть без підключеної камери
# (працює dummy-режим детекцій). Для реальної зйомки обовʼязково передайте
# реальний шлях до пристрою через `CAMERA_DEVICE`.
```
