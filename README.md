...
> **release/0.0.2:** підтримується лише одна Orange Pi Zero; додаткові вузли вилучені з поточного стеку.
...
```mermaid
flowchart LR
    User([👤 Користувач]) -->|природна мова| PC[🖥️ ПК з LLM]
    PC -->|HTTP/WebSocket| OPI[🍊 Orange Pi Zero]
    OPI -->|MQTT control| ARD[📟 Arduino Mega + PCA9685]
    SENS[🛰️ Датчики \nтемпература/вологість/CO₂/освітленість] -->|I2C/Analog| ARD
    ACT[⚙️ Виконавчі \nнасоси/вентилятори/LED/клапани] <-->|PWM/Relay| ARD
...
sequenceDiagram
    ...
    participant PC as 🖥️ ПК (LLM)
    participant OPI as 🍊 Orange Pi Zero
    participant ARD as 📟 Arduino
    participant ACT as ⚙️ Актуатори

    User->>PC: "Підвищити вологість до 60%"
    PC->>OPI: План дій (HTTP/WS)
    OPI->>PC: Поточна телеметрія (MQTT)
    OPI->>ARD: JSON команд (UART)
    ARD-->>OPI: Статус + сенсори (UART)
    OPI-->>PC: Публікація `greenhouse/sensors/#` (MQTT)
    ARD-->>ACT: PWM/Relay сигнали
```
